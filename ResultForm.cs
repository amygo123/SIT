...
string _invCurrentWarehouse = "__ALL__";
        ...
        void BuildHeader()
        {
            ...
            _btnQuery.Click += async (s, e) =>
            {
                ...
                await RefreshInventoryAsync(textNow, force: true);
            };
...
async System.Threading.Tasks.Task RefreshInventoryAsync(string input, bool force = false)
{
    ...
    // cache by input text for TTL seconds
    int ttlSec = _cfg.inventory_cache_ttl_seconds ?? 300;
    if (!force && _invCache.TryGetValue(input, out var cached))
    {
        if ((DateTime.Now - cached.FetchedAt).TotalSeconds <= ttlSec)
        {
...
System.Threading.Tasks.Task RenderInventoryAsync(InventorySnapshot snap)
{
    var rows = snap.Rows;
    ...
    {
        var view = InventoryPivot.FilterByWarehouse(rows, _invCurrentWarehouse);
        var (sumIn, sumOut, low, abn) = InventoryPivot.Summaries(view, _cfg.inventory_low_threshold ?? 10);
...
void ShowWarehousePicker()
{
    if (_invCache.TryGetValue(_boxInput.Text?.Trim() ?? "", out var snap))
    {
        ...
        list.DoubleClick += (s, e) =>
        {
            ...
            dlg.Close();
            _ = RefreshInventoryAsync(_boxInput.Text?.Trim() ?? "", force: false);
        };
